...
以下为当前离线版的真实页面截图，采用手机竖屏视口（390 × 844）。首页标题为「这片儿没我不行」；截图使用全新玩家记录，点击图片可查看大图。

| 绘本首页 | 天空群岛 · 选择地图 | 自然灾害 · 关卡地图 |
| --- | --- | --- |
| [<img src="docs/screenshots/home.png" alt="这片儿没我不行绘本首页与开始游戏按钮" width="240">](docs/screenshots/home.png) | [<img src="docs/screenshots/archipelago.png" alt="天空群岛中的自然灾害、公共安全与居家校园办公三座岛" width="240">](docs/screenshots/archipelago.png) | [<img src="docs/screenshots/map.png" alt="自然灾害地图中的关卡木牌、解锁花苞和右上角工具" width="240">](docs/screenshots/map.png) |

| 关卡介绍 | 找隐患 · 台风居家准备 | 我的进度 |
| --- | --- | --- |
| [<img src="docs/screenshots/level-entry.png" alt="台风居家准备的关卡介绍与开始居家检查按钮" width="240">](docs/screenshots/level-entry.png) | [<img src="docs/screenshots/gameplay.png" alt="台风居家准备真实游戏画面，包含倒计时、提示与暂停按钮" width="240">](docs/screenshots/gameplay.png) | [<img src="docs/screenshots/journal.png" alt="我的进度页中的小红花数量、通关数和三岛进度" width="240">](docs/screenshots/journal.png) |
...
1. **开始游戏**：首次玩家从地图起点出发；已有记录时，首页提供“继续游戏”和“重新开始”，选择“重新开始”会先询问是否清空当前玩家的进度。
2. **选择区域**：点击左下角四分之一圆里的浮岛图标，进入标题为“选择地图”的天空群岛页。点击自然灾害、公共安全或居家校园办公岛屿，旗子移到目的地后进入对应地图；返回按钮保留原来的浏览位置。地图从下方向上探索。
...
| 功能 | 实际行为 |
| --- | --- |
| 开始游戏 / 重新开始 | 首次从地图起点开始；已有记录时，“重新开始”经确认后清空**当前玩家**的花朵、完成记录与解锁进度，保留昵称、地区和其他玩家记录 |
| 取消确认 | 不修改任何存档 |
| 继续游戏 | 读取已有成绩，回到上次探索的地图节点 |
| 重玩已完成关卡 | 可以重新体验，不重复累计首次奖励 |
| 添加 / 切换玩家 | 在本机排行榜操作，各玩家独立保存进度 |

“重新开始”会清空当前玩家的进度，且不会自动备份，请看清确认提示。新一轮进度中，可以重新领取首次通关奖励。
...
更新 README 截图时，先运行 `pnpm export:offline`，再运行 `node scripts/capture-readme-screenshots.mjs`。脚本使用隔离、静音的浏览器，从最终 HTML 采集六个真实页面，保存至 `docs/screenshots/`；[采集记录](docs/screenshots/capture.json) 包含产物校验值、视口和检查结果。
