...
export type Sprite = { image: HTMLImageElement; alpha: Uint8ClampedArray; width: number; height: number };
...
export function loadArt(skin: Skin): Promise<Art> {
  ...
  const pending = Promise.all(Object.entries(skin.assets).map(async ([id, asset]) => {
    ...
    return [id, { image, alpha: ctx.getImageData(0, 0, canvas.width, canvas.height).data, width: canvas.width, height: canvas.height }] as const;
