...
export function PracticePlayer({ pack, onBack, onFinish, journey=false, autoStart=false }: Props) {
  ...
  const reported = useRef(false), reduced = useRef(false), latest = useRef({ run, paused, selected, ready, entered });
  ...
  const interfaceSound = useInterfaceSound(practice ? practiceAudio.settings.muted : audio.settings.muted, practice ? Math.min(1, practiceAudio.settings.sfx / .22) : audio.settings.sfx);
  const soundMuted = practice ? practiceAudio.settings.muted : audio.settings.muted;
  const toggleSound = () => {
    const muted = !soundMuted;
    practiceAudio.setSettings(settings => ({ ...settings, muted }));
    audio.setSettings(settings => ({ ...settings, muted }));
  };
  // Compute this stage's existing hint without dispatching an event into the run.
  const pauseHint = paused ? configuredPauseHint(pack, run) : undefined;
  ...
  const replay = () => { if(practice){practiceAudio.reset();practiceAudio.unlock();}else{audio.reset();audio.unlock();} resetClock.current = true; drag.current = null; reported.current = false; setSelected(null); setPaused(false); setRun(createRun(pack)); };
  const enter = () => { if(!ready || latest.current.entered)return;resetClock.current = true;setEntered(true);if(practice)practiceAudio.unlock();else audio.unlock(); };
  useLevelAutoStart(autoStart, ready, enter, pack.rules.id);
  ...
  return <section {...interfaceSound} ref={surface} style={viewport.style} data-game-surface data-viewport={viewport.viewportKey} onPointerDownCapture={() => { if (entered) { if (practice) practiceAudio.unlock(); else if (pack.skin.response) audio.unlock(); } }} className="configured-player" data-level={pack.rules.id} data-auto-start={autoStart || undefined} data-engine="configured-v1" data-phase={entered ? run.phase : 'ready'} data-emotion={emotion(pack, run)} data-action={run.action?.rule ?? ''} data-resolved={run.resolved.join(',')}
    ...
    <header className="configured-hud painted-game-hud" inert={modal && !paused}>
      <button className="painted-hud-button pause-menu-navigation" onClick={onBack} aria-label="返回关卡"><PaintedIcon name="back" /></button><span>{practice && <small>{`关卡 ${pack.rules.order} · ${run.resolved.length}/${pack.rules.goals.length}`}</small>}{levelTitle(pack.rules.id,pack.rules.title)}</span>
      {!countdown && <time aria-label={elapsedClock?'训练用时（不是救援到达时间）':'风险倒计时'}>{!practice && run.phase !== 'playing' ? (pack.rules.completion.status ?? '安全') : `${String(Math.floor(seconds / 60)).padStart(2, '0')}:${String(seconds % 60).padStart(2, '0')}`}</time>}
      <button className="painted-hud-button" data-ui-sound="hint" aria-label="场景提示" disabled={paused || run.phase !== 'playing'} onClick={() => setRun(r => reduceRun(pack, r, { type: 'hint' }))}><PaintedIcon name="hint" /></button>
      ...
      {countdown && <CountdownBar elapsed={run.elapsed} seconds={pack.rules.risk.seconds} resolved={run.phase === 'settling' || run.phase === 'complete'} training />}
    </header>
    ...
    {entry && (autoStart ? <LevelLaunchStatus error={error} onRetry={reload} onBack={onBack} /> : <PaintedLevelIntro levelId={pack.rules.id} title={levelTitle(pack.rules.id, pack.rules.title)} ready={ready} error={error} onRetry={reload} onStart={enter} onBack={onBack} />)}
    {run.phase === 'failed' && !paused && !entry && !revealingFailure && <PaintedFailure
      levelId={pack.rules.id}
      levelTitle={levelTitle(pack.rules.id, pack.rules.title)}
      kind={run.failure || pack.rules.risk.timeout !== 'fail' ? 'unsafe-action' : 'timeout'}
      reason={run.notice?.text ?? '这次操作触发了风险，请先查看场景中的提示。'}
      hint="先看清这次提醒。救急靠判断，可别硬莽。"
      progress={`已完成 ${run.resolved.length}/${pack.rules.goals.length} 项训练`}
      journey={journey}
      onRetry={replay}
      onBack={onBack}
    />}
    {paused && <PauseMenu hint={pauseHint} muted={soundMuted} onToggleSound={toggleSound} onRestart={replay} onResume={() => { setPaused(false); if (practice) practiceAudio.unlock(); else audio.unlock(); }} />}
    {modal && !paused && run.phase !== 'failed' && !entry && !(journey && completedForReward(run)) && <div className="configured-shade"><div className="configured-dialog" ref={dialog} role="dialog" aria-modal="true" aria-labelledby="configured-dialog-title">
      <h2 id="configured-dialog-title">{run.escaped?'已先行到达高处': (pack.rules.completion.title ?? '这一关，安全了')}</h2>
      {run.escaped?<><p>{pack.rules.interactions.find(rule=>rule.id===run.escapeRule)?.feedback??'已先行转移至安全高处，继续等待救援指引。'}</p><small>安全优先，不为收集物资停留或折返。本次未完成全部训练项目，不发放小红花；可以重新练习。</small></>: <><div className="configured-flowers" aria-label={`获得 ${run.stars} 朵小红花`}>{'✿'.repeat(run.stars)}</div><p>{pack.rules.completion.summary}</p>{practice&&pack.rules.completion.status&&<small>{pack.rules.completion.status}</small>}</>}
      {!practice && pack.skin.response && <button data-ui-sound="off" onClick={() => audio.setSettings(s => ({ ...s, muted: !s.muted }))}>{audio.settings.muted ? '打开声音' : '关闭声音'}</button>}
