/** Offline integration: isolated fixtures unlock only the map; all goals use real pointer/touch input. */
...
await dependency('esbuild').build({
  absWorkingDir: root, stdin: { resolveDir: root, contents: [
    ...
    "export {default as rules} from './content/levels/flood-highground-practice/level.json';",
    "export {default as skin} from './content/levels/flood-highground-practice/skins/paper-gouache.json';",
...
const out = path.resolve(root, process.argv[3] ?? 'docs/flood-refinement/verification/browser');
