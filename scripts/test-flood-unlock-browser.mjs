...
try{
 ...
 await node.click();await page.getByRole('button',{name:'进入场景',exact:true}).click();await page.waitForFunction(()=>document.querySelector('.configured-player')?.dataset.ready==='true');
 await page.locator('.configured-dialog').getByRole('button',{name:'进入场景',exact:true}).click();assert.equal(await page.locator('.configured-player').getAttribute('data-level'),'flood-highground-practice');
