...
const out=path.join(root,'docs/flood-refinement/verification');fs.mkdirSync(out,{recursive:true});
const scripts=JSON.parse(fs.readFileSync(path.join(root,'package.json'))).scripts;
const jobs=scripts.test.split(' && ').map(s=>{if(!s.startsWith('node '))throw Error('Unexpected runner');return s.split(' ').slice(1);});
jobs.push(['scripts/test-flood-refinement.mjs'],['scripts/test-flood-presentation.mjs'],['scripts/test-flood-audio.mjs'],['scripts/test-fire-stairs.mjs'],['scripts/test-fire-shelter.mjs'],['scripts/test-typhoon-immersion.mjs'],['node_modules/typescript/bin/tsc','--noEmit']);
